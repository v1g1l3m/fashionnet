...
def save_bottleneck_df(num_per_file):
    img_name_class_attr_bbox_iou_part = []
    for train_val in ['validation', 'train']:
        with open(os.path.join(fashion_dataset_path, train_val + '85.txt')) as f:
            for line in f:
                line = line.split()
                img_path = line[0]
                ...
                img_bbox.append(iou)
                img_name_class_attr_bbox_iou_part.append((img_path, class_1_hot, attrs_1_hot, img_bbox, img_part))
                            
    shuffle(img_name_class_attr_bbox_iou_part)
     ## Build the VGG16 network
    model = VGG16(include_top=False, weights='imagenet', input_shape=input_shape)                                            
    for train_val in ['train', 'validation']:
        ...
        index = 0
        with open(os.path.join(btl_path, 'btl_' + train_val + '_npz.txt'), 'w') as f_image:
            for img_path, class_1_hot, attrs_1_hot, img_bbox_iou, part in img_name_class_attr_bbox_iou_part:
                if part != train_val:
                    continue
                current_size = len(images_list)
...
if __name__ == '__main__':
    ...
    # shuffle(img_name_class_attr_iou_tuples)
    # images_list = []
    # classes_idx = dict((x, 0) for x in class35)
