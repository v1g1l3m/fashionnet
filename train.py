...
btl_path = os.path.join(fashion_dataset_path, 'bottleneck')


def create_model(is_input_bottleneck, input_shape, classes, attributes, mode):
    ...
    x = Dense(512, activation='elu', name='dense_2_bbox')(x)
    x = BatchNormalization(name='bn_2_bbox')(x)
    ...
    x = Dense(512, activation='elu', name='dense_2_attr')(x)
    x = BatchNormalization(name='bn_2_attr')(x)
...
def train_model():                 
    ...
    train_steps = 3
    epochs = 2
    with Parallel_np_arrays_reader(os.path.join(btl_path, 'btl_train_npz.txt'), ['bbiou', 'attr', 'cls'], maxsize=int(0.15*train_steps), numproc=1) as train_gen:
    ...
    #     temp = np.load(os.path.join(btl_path, 'test5.npz'))
        temp = np.load(os.path.join(btl_path, 'validation/btl_validation_256_0000000.npz'))
        X = np.array(temp['btl'])
        ...
        val_data=(X, [Yb, Ya, Yc])
        with open(os.path.join(fashion_dataset_path, 'attr_data_train85.pkl'), 'rb') as f:
            train_labels_attr = pickle.load(f)
        with open(os.path.join(fashion_dataset_path, 'class_data_train85.pkl'), 'rb') as f:
        ...
        checkpoint = ModelCheckpoint(filepath, monitor='predictions_attr_acc', verbose=1, save_best_only=True, save_weights_only=False, mode='auto', period=5)
        ...
        model = create_model(True, (7, 7, 512), class35, attr200, mode='bac')
